...
(() => {
  ...
  const methodNames = {pe_band:'本益比區間（與自己過去一年比較）',pb_band:'股價淨值比區間（與自己過去一年比較）',etf_band:'ETF 一年價格位置',forward_pe:'預估本益比',normalized_pe:'正常化本益比'};
  ...
  // Quality holds survive plan expiry and missing risk/quote data.
  const gap=i=>i.valuation_quality?.status==='needs_research'||['valuation_gap','overvalued'].includes(i.plan?.status);
  const qualityReason=i=>i.valuation_quality?.reason||i.plan?.reason||'價位需重新評估';
  const displaySignal=i=>{if(gap(i))return {...signal(i),status:'blocked',suggested:null,action:qualityReason(i)};const p=plan(i);return p?{...signal(i),status:p.band,action:p.reason}:signal(i);};
  ...
  function verdictHTML() {
    ...
    const ready=ps.filter(([,p])=>p.status==='conditional'),go=ready.filter(([i])=>researched(i)),pool=ready.filter(([i])=>!researched(i)),wait=ps.filter(([i,p])=>p.status==='wait_stabilize'&&researched(i));
    ...
    const body=go.length?`<p class="verdict-head go">${day}可考慮進場</p>${stocks.length?`<ul>${stocks.map(li).join('')}</ul>`:'<p>個股：有研究支持的標的目前沒有進入買進區。</p>'}${etfs.length?`<p class="muted">ETF</p><ul>${etfs.map(li).join('')}</ul>`:''}<p class="muted">全市場依收盤規劃，由便宜到貴排列。08:50 Discord 會用當日試撮再確認，沒收到確認就不要掛單；是否下單由你決定。</p>`
      :`<p class="verdict-head stop">${day}不進場</p><p>${wait.length?`有研究支持的標的沒有符合全部條件。${names(wait)} 價格已進入買進區，但要先等止穩。`:data.items.some(i=>gap(i)&&researched(i))?'部分關注股的價位仍需重新評估；其餘有研究支持的標的未符合進場條件。':'有研究支持的標的都不在合理買進區。'}</p>`;
  ...
  function researched(i) {
    if(i.kind!=='stock')return true;
    const r=i.research_detail;
    return !!(r?.thesis&&r?.risks?.length&&timeValue(r.as_of)<=Date.now()&&timeValue(r.valid_until)>Date.now());
  }
  ...
  function render() {
    ...
    if(candidates){
      const eligible=data.mode==='live'&&!loadError?data.items.filter(i=>plan(i)?.entry&&researched(i)).sort((a,b)=>Number(plan(a).status==='wait_stabilize')-Number(plan(b).status==='wait_stabilize')||discount(b)-discount(a)):[];
      $('candidates-section').hidden=!eligible.length;
      candidates.innerHTML=eligible.length?eligible.slice(0,6).map(i=>{const p=plan(i),r=i.research_detail||{};return `<article class="health-card plan-card"><span class="tag ${p.status==='conditional'&&researched(i)?'buy':''}">${esc(p.label)}</span>${researched(i)?'':'<span class="tag">尚無個股研究</span>'}<h3><button class="stock-name" data-symbol="${esc(i.symbol)}">${esc(i.name)} ${esc(i.symbol)}</button></h3><p class="plan-price">承接參考 <strong>${price(p.entry)}</strong><span> 元</span></p><p>最高接受 <strong>${price(p.buy_max)} 元</strong> · 超過不追</p><p>${esc(p.reason)}。</p><p>${esc(r.thesis)}</p>${purchaseReasons(i)}<p class="watch-risk">風險：${esc(r.risks?.[0]||'待確認')}</p><small>${esc(p.close_date)} 收盤規劃 · 更新期限 ${esc(clockText(p.valid_until))}<br>盤前確認後才考慮掛單</small></article>`;}).join(''):'<p>目前没有完整有效的承接計畫，請查看下方等待原因。</p>'.replace('没有','沒有');
    ...
    $('rows').innerHTML=items.slice(0,visibleLimit).map(i=>{
      const s=signals.get(i.symbol),v=gap(i)?{}:i.valuation||{},q=i.quote||{};
      return `<tr><td>${star(i)}<button class="stock-name" data-symbol="${esc(i.symbol)}">${esc(i.name||i.symbol)}<span class="code">${esc(i.symbol)}</span></button><div class="desc" title="${esc(i.description||'公司／ETF 介紹待補')} "><span class="kind">${kindName(i.kind)}</span>${esc(i.description||i.industry||'介紹待補')}</div></td><td title="${esc(q.previous_close_date||q.trade_date||'資料日未提供')}">${price(publicClose(i)?.close ?? (quoteToday(q)?q.previous_close:null))}${publicClose(i)?`<div class="action">${esc(publicClose(i).trade_date)} · 收盤</div>`:!quoteToday(q)&&positive(q.previous_close)?'<div class="action">舊行情，待更新</div>':''}</td><td title="${esc(clockText(q.trial_as_of||q.as_of))}">${price(quoteToday(q)?q.trial_price:null)}</td><td class="suggest-cell ${positive(s.suggested)?'':'missing'}">${plan(i)?.entry?price(plan(i).entry):positive(s.suggested)?price(s.suggested):'暫不掛'}</td><td class="anchor">${price(v.sweet)}</td><td class="anchor">${price(v.add)}</td><td class="anchor">${price(v.buy)}</td><td><span class="tag ${s.status}">${labels[s.status]}</span><div class="action">${esc(!i.valuation&&i.fundamentals?i.fundamentals.reading:s.action||s.reason||'等待確認')}</div></td><td class="date-cell">${gap(i)?'價位重估中':i.valuation?'價格分析':i.fundamentals?'基本面觀察':'資料待補'}</td></tr>`;
    }).join('');
    bindStars($('rows'));
  ...
  function detailHTML(i) {
    ...
    const tiles=[['承接參考（待盤前確認）',plan(i)?.entry],['最近公開收盤',publicClose(i)?.close ?? (quoteToday(q)?q.previous_close:null)],['盤前試撮',quoteToday(q)?q.trial_price:null],['甜甜價',gap(i)?null:v.sweet],['加碼價',gap(i)?null:v.add],['買進價',gap(i)?null:v.buy]];
    return `<div class="detail-head">${star(i)}<h2 id="detail-title">${esc(i.name||i.symbol)} <span class="code">${esc(i.symbol)} · ${kindName(i.kind)}</span></h2><p>${esc(i.description||'公司／ETF 介紹待補')}</p><span class="tag ${s.status}">${labels[s.status]}</span><p>${esc(s.action||s.reason)}</p></div><div class="detail-prices">${tiles.map(([label,n],j)=>`<div class="price-block ${j===0?'primary':''}"><span>${label}</span><strong>${price(n)}</strong></div>`).join('')}</div><p class="muted">價格依據 ${esc(publicClose(i)?.trade_date||'未取得')} 收盤；承接計畫有效至 ${esc(clockText(plan(i)?.valid_until))}。當日試撮與五檔由 Discord 盤前通知確認。</p>${purchaseReasons(i)}${basicHTML(i)}<div class="research-grid">${textSection('為什麼看好',r.thesis||v.thesis)}${textSection('研究失效條件',v.invalidation?.join('；')||i.research_detail?.invalidation?.join('；'))}${textSection('為什麼現在買／不買',plan(i)?.reason||r.why_now||s.reason)}${textSection('籌碼',r.chips)}${listSection('催化劑',r.catalysts)}${listSection('主要風險',r.risks)}${textSection('估值方法',gap(i)?qualityReason(i):v.method ? `${methodNames[v.method]||v.method}；${v.reason||''}`:null)}${textSection('其他承接位置',positive(s.suggested)?`保守撿價 ${price(s.conservative)}；極端承接 ${price(s.extreme)}。未有價格結構支持的欄位留空。`:null)}${textSection('不追與減碼',!gap(i)&&(positive(v.avoid)||positive(v.sell)||positive(v.reduce))?`不追 ${price(v.avoid)}；開始賣 ${price(v.sell)}；強減碼 ${price(v.reduce)}`:null)}</div><section class="sources"><h3>研究來源</h3>${sourceLinks(r.sources?.length?r.sources:v.sources)}</section>`;
  }
