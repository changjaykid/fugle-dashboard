...
(() => {
  ...
  function stats(signals) {
    ...
    else if(valid===0){notice.textContent=`目前沒有資料完整且仍有效的掛價。研究觀察 ${data.coverage?.watched??data.items.filter(i=>i.watched).length} 檔；已套用估值 ${data.coverage?.valued??0} 檔。下方可看研究與待核准價位，這不代表市場沒有機會。`;}
  ...
  function render() {
    ...
    $('rows').innerHTML=items.slice(0,visibleLimit).map(i=>{
      ...
      return `<tr><td><button class="stock-name" data-symbol="${esc(i.symbol)}">${esc(i.name||i.symbol)}<span class="code">${esc(i.symbol)}</span></button><div class="desc" title="${esc(i.description||'公司／ETF 介紹待補')} "><span class="kind">${kindName(i.kind)}</span>${esc(i.description||'介紹待補')}</div></td><td title="${esc(q.previous_close_date||q.trade_date||'資料日未提供')}">${price(publicClose(i)?.close ?? (quoteToday(q)?q.previous_close:null))}${publicClose(i)?`<div class="action">${esc(publicClose(i).trade_date)} · 收盤</div>`:!quoteToday(q)&&positive(q.previous_close)?'<div class="action">舊行情，待更新</div>':''}</td><td title="${esc(clockText(q.trial_as_of||q.as_of))}">${price(quoteToday(q)?q.trial_price:null)}</td><td class="suggest-cell ${positive(s.suggested)?'':'missing'}">${positive(s.suggested)?price(s.suggested):'暫不掛'}</td><td class="anchor">${price(v.sweet)}</td><td class="anchor">${price(v.add)}</td><td class="anchor">${price(v.buy)}</td><td><span class="tag ${s.status}">${labels[s.status]}</span><div class="action">${esc(s.action||s.reason||'等待確認')}</div></td><td class="date-cell">${dateText(v.as_of)}</td></tr>`;
    }).join('');
  ...
  function renderResearch() {
    ...
    box.innerHTML=rows.length?rows.map(i=>{const r=i.research_detail,p=r.proposal,c=publicClose(i),valid=timeValue(r.valid_until)>Date.now()&&timeValue(r.as_of)<=Date.now();return `<article class="research-card"><div class="research-top"><button class="stock-name" data-symbol="${esc(i.symbol)}">${esc(i.name)} <span class="code">${esc(i.symbol)}</span></button><span class="tag">${valid?'研究觀察':'研究過期'}</span></div><p class="muted">${esc(r.description)}</p><h3>${esc(valid?r.stance:'先更新研究，不引用舊判斷')}</h3><p>${esc(r.thesis)}</p><div class="research-facts"><span>最近收盤 <strong>${price(c?.close)}</strong><small>${esc(c?.trade_date||'尚未取得')} · 非即時</small></span><span>半年 EPS <strong>${price(r.facts?.h1_eps)}</strong><small>${esc(r.facts?.period)}</small></span></div>${p&&valid?`<div class="proposal-preview"><strong>待核准提案 · ${esc(p.id)}</strong><div class="draft-prices"><span>甜甜 ≤ <b>${price(p.sweet)}</b></span><span>加碼 ≤ <b>${price(p.add)}</b></span><span>買進 ≤ <b>${price(p.buy)}</b></span></div><p>僅在估值假設成立時使用；目前不可依此掛單。</p><details><summary>看算式、情境與限制</summary><p>H1 EPS ${price(p.inputs?.h1_eps)} × 2 × 樣本 PE ${price(p.inputs?.sample_median_pe)} = 基準 ${price(p.fair)} 元。</p><p>EPS ±20% 敏感度：${price(p.scenario_low)}～${price(p.scenario_high)} 元；不代表機率。</p><p>${esc(p.reason)}</p><p>${esc(p.limitations)}</p><p>有效至 ${esc(clockText(p.valid_until))}。</p>${sourceLinks(p.sources)}</details></div>`:`<div class="proposal-preview">${esc(r.valuation_gap||'研究過期，價位暫停引用')}</div>`}<p><strong>主要風險</strong> ${esc(r.risks?.join('；'))}</p><p class="muted">籌碼與量價尚未整合；無已確認的技術支撐。</p><button class="quiet" data-symbol="${esc(i.symbol)}">完整依據與來源 ↗</button><small>研究更新 ${esc(clockText(r.as_of))}</small></article>`;}).join(''):'<p>研究資料尚未接入，暫無可查閱內容。</p>';
  ...
  function detailHTML(i) {
    ...
    const tiles=[['今日建議掛價',s.suggested],['最近公開收盤',publicClose(i)?.close ?? (quoteToday(q)?q.previous_close:null)],['盤前試撮',quoteToday(q)?q.trial_price:null],['甜甜價',v.sweet],['加碼價',v.add],['買進價',v.buy]];
    return `<div class="detail-head"><h2 id="detail-title">${esc(i.name||i.symbol)} <span class="code">${esc(i.symbol)} · ${kindName(i.kind)}</span></h2><p>${esc(i.description||'公司／ETF 介紹待補')}</p><span class="tag ${s.status}">${labels[s.status]}</span><p>${esc(s.action||s.reason)}</p></div><div class="detail-prices">${tiles.map(([label,n],j)=>`<div class="price-block ${j===0?'primary':''}"><span>${label}</span><strong>${price(n)}</strong></div>`).join('')}</div><p class="muted">行情時間 ${clockText(q.as_of)} · 來源 ${esc(q.source||'未取得')}<br>該筆行情現價 ${price(q.price)} · 參考價 ${price(q.reference_price)} · 行情日 ${esc(q.trade_date||'未提供')}<br>掛價效期 ${s.suggested?clockText(s.valid_until):'目前無有效掛價'} · 估值日期 ${dateText(v.as_of)}</p><div class="research-grid">${textSection('為什麼看好',r.thesis||v.thesis)}${textSection('研究失效條件',i.research_detail?.invalidation?.join('；'))}${textSection('為什麼現在買／不買',r.why_now||s.reason)}${textSection('籌碼',r.chips)}${listSection('催化劑',r.catalysts)}${listSection('主要風險',r.risks)}${textSection('估值方法',v.method ? `${v.method}；${v.reason||''}`:null)}${textSection('其他承接位置',positive(s.suggested)?`保守撿價 ${price(s.conservative)}；極端承接 ${price(s.extreme)}。未有價格結構支持的欄位留空。`:null)}${textSection('不追與減碼',positive(v.avoid)||positive(v.sell)||positive(v.reduce)?`不追 ${price(v.avoid)}；開始賣 ${price(v.sell)}；強減碼 ${price(v.reduce)}`:null)}</div><section class="history"><h3>估值歷史</h3>${i.valuation_history?.length?i.valuation_history.map(h=>`<article><strong>${dateText(h.created_at)}<span class="tag">${esc({active:'已套用',proposed:'待套用',superseded:'歷史版本',rejected:'未採用'}[h.status]||h.status)}</span></strong><p>甜甜 ${price(h.payload?.sweet)} / 加碼 ${price(h.payload?.add)} / 買進 ${price(h.payload?.buy)}</p><p>${esc(h.payload?.reason||'未提供修改原因')}</p></article>`).join(''):'<p>尚無估值版本。研究提案套用後會保留修改原因。</p>'}</section><section class="sources"><h3>研究來源</h3>${sourceLinks(r.sources?.length?r.sources:v.sources)}</section>`;
  }
  ...
  function system() {
    ...
    $('coverage').textContent=`母表 ${(c.universe??data.items.length).toLocaleString()} · 有估值 ${c.valued??0} · 公開收盤 ${c.public_closes??0} · 研究 ${c.research??0} · 即時行情 ${c.quotes??0}`;
