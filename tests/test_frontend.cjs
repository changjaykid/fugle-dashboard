...
const fixture=()=>({schema_version:1,mode:'simulation',generated_at:iso(0),items:[{symbol:'TEST',name:'<script>unsafe</script>',kind:'stock',quote:{trade_date:"2026-09-10",book_as_of:iso(0),as_of:iso(0),is_trial:true,trial_price:96},valuation:{buy:100,valid_until:iso(3600000),evidence_reviewed:true},signal:{status:'buy',suggested:95,calculated_at:iso(0),valid_until:iso(120000)}}]});
...
test('prior-day quotes cannot be labelled yesterday close or today trial',async()=>{const d=fixture();d.items[0].quote.trade_date='2026-09-09';d.items[0].quote.previous_close=123.45;const a=await app(d);assert.doesNotMatch(a.node('rows').innerHTML,/>123.45</);assert.match(a.node('rows').innerHTML,/舊行情/);assert.match(a.node('rows').innerHTML,/暫不掛/);});
test('source older than 120s cannot be rescued by fresh signal expiry',async()=>{const d=fixture();d.items[0].quote.as_of=iso(-121000);const a=await app(d);assert.match(a.node('rows').innerHTML,/暫不掛/);});
